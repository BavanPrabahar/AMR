...
class Hiyaah(Node):
    ...
    def ab(self):
        ...
        poses = [
            self.tf_conv(27.1,3.09,1.57),
            self.tf_conv(10.9,18.2,2.355),
            self.tf_conv(-7.54,22.9,0.0),
            self.tf_conv(-7.54,22.9,3.14),
            self.tf_conv(-24.7,7.02,-1.57),
            self.tf_conv(-32.9,1.18,-1.57),
            self.tf_conv(-35.4,-2.68,-0.785),
            self.tf_conv(-17.6,-22.9,-0.785),
            
            self.tf_conv(-2.66,-3.2,1.57),
            self.tf_conv(-0.197,0.409,1.57)
        ...
        if self.way.data:
            ...
            while i<len(self.way.data):
                if (self.pos.pose.position.x - 0.05 <= self.gooo.position.x and self.gooo.position.x < self.pos.pose.position.x + 0.05) and \
                    ...
                    self.start=0
                    hell.goToPose(self.tf_conv(9.3,-0.072,0))
                    i=-1
